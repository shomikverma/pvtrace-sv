...
class OptionalMirrorAndSolarCell(FresnelSurfaceDelegate):
    ...
    def reflectivity(self, surface, ray, geometry, container, adjacent):
        ...
        if np.allclose((0, 0, -1), normal) and want_back_surface_mirror:
            ...
            return 0.0
        return super(OptionalMirrorAndSolarCell, self).reflectivity(surface, ray, geometry, container, adjacent)  # opt-out of handling custom reflection
    ...
    def transmitted_direction(self, surface, ray, geometry, container, adjacent):
        ...
        return super(OptionalMirrorAndSolarCell, self).transmitted_direction(surface, ray, geometry, container, adjacent)  # opt-out of handling custom reflection
...
class LSC(object):
    ...
    def _make_scene(self):
        ...
        lsc = Node(
            ...
            geometry=Box(
                (l, w, d),
                material=Material(
                    refractive_index=self.n1, components=components,
    ...
    def add_air_gap_mirror(self, lambertian=False):
        self._air_gap_mirror_info = {'want_air_gap_mirror': True, 'lambertian': lambertian}
    ...
    def _make_counts(self, df):
        ...
        for facet in {'left', 'right', 'near', 'far', 'top', 'bottom'}:
            solar_out[facet] = self.spectrum(facets={facet}, source=all_lights, kind='last').shape[0]
            
        # Count solar photons entering
        solar_in = dict()
        for facet in {'left', 'right', 'near', 'far', 'top', 'bottom'}:
            solar_in[facet] = self.spectrum(facets={facet}, source=all_lights, kind='first').shape[0]
    
        # Count luminescent photons exiting
        lum_out = dict()
        for facet in {'left', 'right', 'near', 'far', 'top', 'bottom'}:
            lum_out[facet] = self.spectrum(facets={facet}, source=all_components, kind='last').shape[0]
        
        # Count luminescent photons entering
        lum_in = dict()
        for facet in {'left', 'right', 'near', 'far', 'top', 'bottom'}:
            lum_in[facet] = self.spectrum(facets={facet}, source=all_components, kind='first').shape[0]
    ...
    def spectrum(self, facets=set(), kind='last', source='all', events=None):
        ...
        if isinstance(facets, (list, tuple, set)):
            ...
            raise ValueError("`'facets'` should be a set `{'left', 'right'}`", {'got': facets})
        ...
        if events is None:
            ...
            if isinstance(events, (list, tuple, set)):
                ...
                if events.issubset(all_events):
                    ...
                    raise ValueError("Contained some unknown events", {"got": events, "expected": all_events})
            else:
                raise ValueError("Events must be set of event strings", {"allowed": all_events})
    ...
    def summary(self):
        ...
        s = pd.Series({
            ...
            "Waveguide Efficiency (Thermodynamic Prediction)": (n**2/(Cg+n**2)),
...
def test1():
    ...
    incident = lsc.spectrum(source={"Oriel Lamp + Filter"}, kind='first', facets={'top'})
    hitting = len(incident)
    counts= {"edge": len(edge), "escape": len(escape), "lost": len(lost)}
    fractions = {"edge": counts["edge"]/hitting, "escape": counts["escape"]/hitting, "lost": counts["lost"]/hitting}
